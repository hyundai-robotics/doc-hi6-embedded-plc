...
내장 PLC가 Stop될 때에는 PLC의 출력신호인 DI, Y 릴레이가 자동으로 0이 됩니다. *(DI는 로봇언어나 할당의 관점에서는 이력이지만, 내장PLC의 관점에서는 출력입니다.)*  
다만, PLC 외의 HRScript나 할당에서도 Y출력에 값을 출력할 수 있는 조건인 SP11=On인 경우에는 Y출력은 이전 값을 유지합니다.
