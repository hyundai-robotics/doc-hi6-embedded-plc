...
각각의 type은 뒤에서 자세히 설명됩니다.
  ...
  9) R (auxiliaRy) : 값을 임시로 보관하기 위한 보조 릴레이입니다. 

  10) K (Keep) : 값을 임시로 보관하기 위한 보조 릴레이이며 전원을 꺼도 값이 보관됩니다. 

  11) T (Timer) : 타이머 동작을 위한 릴레이며, 값이 0일 때 접점이 On됩니다. 

  12) C (Counter) : 카운터 동작을 위한 릴레이며, 값이 0일 때 접점이 On 됩니다.  
